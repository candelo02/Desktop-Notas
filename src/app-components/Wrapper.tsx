...
export default React.memo((props: any) => {
    ...
    const handle_delete_note = React.useCallback(() => {
        if (!active_note || active_note.id == null) return; // evita llamar si es null
    ...
    React.useLayoutEffect(() => {
        const handler = (ev: Event) => {
            const custom = ev as CustomEvent<INoteData[]>;
            console.log("ev", custom.detail);  
            const sorted_note = custom.detail.sort(sort_notes)
            custom.detail.length === 0 ? set_state('active_note', null) : set_state('active_note', sorted_note[0]);
            set_state('notes', sorted_note)
        };
        window.addEventListener('all-notes-data', handler as EventListener);
        return () => window.removeEventListener('all-notes-data', handler as EventListener);
    }, [])
