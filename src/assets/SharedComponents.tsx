...
export const NotesItem = React.memo((props: {note: INoteData; onClick: Function}) => {
    ...
    return (
        <div 
            onContextMenu={() => props.note.id != null && window.electron.open_note_item_context_menu(props.note.id.toString())} 
            className={clsx('w-[100%] p-4 [&.active]:rounded-2xl cursor-pointer', {'active': active_note?.id == props.note.id})} 
